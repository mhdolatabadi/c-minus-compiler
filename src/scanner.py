...
# Lexical errors must be recorded in a text file called 'lexical_errors.txt'. each error is reported on a separate line as a pair including string and a message with the line number.
# if the input program lexically correct, sentece 'There is no lexical error.' should be written.
...
#    * if comment remains open when end of the input file is ecountered, record this with just the message 'Unclosed comment'. it is sufficient to print at most the first seven characters
#    of the unclosed comment with three dots.
...
# Scanner should maintain a variable that indicates which line of the input text is currently being scannded. this variable is changed whenever scanner reads a new line symbol (\n).
# there is no need to record or report COMMENT and WHITESPACE tokens.
...
keywordRegex = r"if|else|void|int|repeat|break|until|return"
symbolRegex = r";|:|,|\[|\]|\(|\)|{|}|\+|-|\*|=|<|==" 
numberRegex = r"[0-9]+"
...
for line in input_file:
    print(line_number)
    print("keywords: ", find_pattern(line, keywordRegex))
    print("symbols: ", find_pattern(line, symbolRegex))
    print("numbers: ", find_pattern(line, numberRegex))
    print("identifiers: ", find_pattern(line, identifierRegex))
    print("comments: ", find_pattern(line, commentRegex))
    print("whitespaces: ", find_pattern(line, whitespaceRegex))
    line_number += 1
